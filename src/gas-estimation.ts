 ...
 */
const DEFAULT_GAS_ESTIMATION_OPTIONS: Required<GasEstimationOptions> = {
  bufferPercentage: 20,
...
export async function estimateUserOperationGas(
  ...
  try {
    ...
    const callGasLimit = await estimateGas(
      client,
      {
        from: userOp.sender,
        to: userOp.sender,
    ...
    throw new GasEstimationError(`Failed to estimate user operation gas: ${(error as Error).message}`);
  }
}
...
export function createGasEstimationMiddleware(
  ...
  return {
    gasEstimator: async (userOp: any) => {
      logger.debug('Gas estimation middleware called', userOp);
      
      try {
        // Estimate gas parameters
        const gasParams = await estimateUserOperationGas(
          client,
          entryPointAddress,
          {
        ...
        throw new GasEstimationError(`Gas estimation middleware failed: ${(error as Error).message}`);
      }
    },
  };
}